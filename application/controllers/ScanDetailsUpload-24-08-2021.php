	...
	**/
				...
				foreach($obj->getWorksheetIterator() as $sheet){
					...
					for($row = $startRow; $row <= $getHighestRow; $row++){
						...
						$InsertData = array(
							...
							'mode'   		=> $sheet->getCellByColumnAndRow(3,$row)->getValue(),
							'starting_location_no'  => $sheet->getCellByColumnAndRow(4,$row)->getValue(),
							'mapping_qty' 	=> $sheet->getCellByColumnAndRow(5,$row)->getValue(),
							'created_on' 	=> date('Y-m-d')
						);
	...
	public function deleteScanItem(){
		...
		if( isset($param) && !empty($param)){
			...
			if( isset($param) && !empty($param) && $param['store_code']!='' && $param['from_date']!='' ){
				...
				$deleteMsg = ' with store code '. $store_code. ' and date '.$from_date;
			}
			...
			//$this->CommonModel->DeleteAllPermanently($whereConditions, TBL_VARIANCE_RECONCILATION_UPLOAD);
	...
	public function deleteItemMaster(){
		...
		if( isset($param) && !empty($param)){
			...
			$itemData = $this->CommonModel->RetriveRecordByWhere(TBL_ITEM_MASTER, array("created_on" => date('Y-m-d',strtotime($param['from_date']))));
